...
import React, { useContext, useEffect, useState } from "react";
import { FiPlus, FiX, FiArrowLeft, FiTrash2 } from "react-icons/fi";
import Card from "../components/Card";
import { NotebookContext } from "../../context/NotebookContext";
import { useParams, Link, useNavigate } from "react-router-dom";
...
const AllNotes = () => {
  ...
  const { getNotesByNotebookId, createNote, deleteNotebook, setId } =
  ...
  return (
    <div className="h-screen flex justify-center items-center">
      ...
      <button
        ...
        className="fixed top-5 right-10 flex items-center gap-2 bg-[#C85454] hover:bg-[#b94f4f] text-white px-3 py-2 rounded-md cursor-pointer transition z-50 shadow-md"
      >
        <FiTrash2 size={20} />
      ...
      {!display && (
        <div className="fixed bottom-10 right-10 bg-[#C85454] flex justify-center items-center h-[65px] w-[65px] rounded-full md:h-[100px] md:w-[100px]">
          <FiPlus
            ...
            className="text-white text-[35px] md:text-[60px] cursor-pointer"
          />
        </div>
      )}
